...
- Runner and credential boundary. The local supervisor is the only V1 runner surface. Managed background mode is local and controllable with `--status` and `--stop`. Status includes foreground/watch activity and current work details. Manual guarded ticks are for debugging. Codex Automations, Claude `/loop`, hosted CI/bot runners, cron, and launchd are outside the V1 default.
...
- First trial. Normal setup creates the safe smoke-test issue from `.agent-loops/FIRST-TRIAL-ISSUE.md`. If the smoke issue reaches `ready-for-human` or `ready-for-human-baseline-red`, setup closes the smoke issue, closes the smoke PR, and requests deletion of the smoke branch. Failed smoke artifacts remain open for diagnosis. For manual setup, point the user at that file and show both the guided one-command path and the manual commands.
