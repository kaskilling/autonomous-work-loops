...
Goal: review one changed head adversarially, anchored to proof, diff facts, hosted checks, and external inline bot review comments, then route to `needs-fix`, `ready-for-human`, `checks-pending`, `unproven`, or `did-not-converge`.
...
13. If only non-blocking defects remain at the cycle cap, call `post_marker` with `verdict=ready-for-human`, list remaining items, call `set_label` to `ready-for-human`, and exit.
14. If blocking defects remain at the cycle cap, call `post_marker` with `verdict=did-not-converge`, call `set_label` to `did-not-converge` (its own terminal label, NOT `ready-for-human`), list the blocking items, route to human, and exit.
15. If the head is clean, proof passed, external inline review is clear, and hosted checks are green or classified as default-branch baseline failures, call `post_marker` with `verdict=ready-for-human` and call `set_label` to `ready-for-human`. A clean first pass converges immediately after host-state classification — do NOT force a fixer cycle when there is nothing to fix.
