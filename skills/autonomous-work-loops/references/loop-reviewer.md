...
9. If local proof and review are clean, wait for hosted PR checks up to the configured wait budget, then read external inline bot review comments for the current head.
10. If hosted checks are still pending, call `post_marker` with `verdict=checks-pending`, leave the PR in non-terminal state, and exit so a later tick can re-check.
11. If external inline bot review comments exist for the current head, route to `needs-fix` and include the comment summary.
12. If hosted checks failed and the same check name is not failing on the default branch, route to `needs-fix`.
13. If hosted checks failed only where the same check names are failing on the default branch, call `post_marker` with `verdict=ready-for-human-baseline-red`, call `set_label` to `ready-for-human-baseline-red`, and include the baseline classification.
14. If only non-blocking defects remain at the cycle cap, call `post_marker` with `verdict=ready-for-human`, list remaining items, call `set_label` to `ready-for-human`, and exit.
15. If blocking defects remain at the cycle cap, call `post_marker` with `verdict=did-not-converge`, call `set_label` to `did-not-converge` (its own terminal label, NOT a human-handoff label), list the blocking items, route to human, and exit.
16. If the head is clean, proof passed, external inline review is clear, and hosted checks are green, skipped, neutral, or absent, call `post_marker` with `verdict=ready-for-human` and call `set_label` to `ready-for-human`. A clean first pass converges immediately after host-state classification; do not force a fixer cycle when there is nothing to fix.
17. At the end of a normal review, run the evidence threshold check from `evidence-capture.md`. If threshold is met, open one tiny playbook-suggestion PR.
