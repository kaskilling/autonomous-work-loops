...
| V1 fresh setup/docs | setup/UX | PASS | Static audit now requires `gh auth`, authenticated login, safe `trusted_actors`, label `--force` commands, the single foreground supervisor surface, and no Codex Automations, Claude `/loop`, cron, or GitHub Actions setup path |
